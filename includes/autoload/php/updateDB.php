<?php
  function updateDB(){
    ...
    $stmt = "SELECT c.categories_id, cd.categories_name, c.parent_id, l.standard, l.barcode, l.game_case, l.game_sleeve
         FROM ".TBL_CATEGORIES." c
         LEFT JOIN ".TBL_CATEGORIES_DESCRIPTION." cd ON c.categories_id = cd.categories_id
         LEFT JOIN ".TBL_LABELS." l ON c.categories_id = l.categories_id
    ...
    $stmt2 = $mysqlL->prepare("INSERT INTO categories
                               SET categories_id = ?,
                                   categories_name = ?,
                                   parent_id = ?");
    $stmt3 = $mysqlL->prepare("INSERT INTO labels
                               SET categories_id = ?,
                                   standard = ?,
                                   barcode = ?,
                                   game_case = ?,
    ...
    if($result->num_rows > 0){
      while($row = $result->fetch_array(MYSQLI_ASSOC)){
        ...
        $stmt3->bind_param("iiiii", $row['categories_id'], $row['standard'], $row['barcode'], $row['game_case'], $row['game_sleeve']);
        $stmt3->execute();
      }
    }
    $stmt2->close();
